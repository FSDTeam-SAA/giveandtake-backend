...
const AI_TEXT_STOPWORDS = new Set([
  "a",
  "about",
  "above",
  "after",
...
class JobFitService {
  ...
  async evaluate(
    ...
    const compactJobText =
      this.compactForGeminiText(jobText) || jobText;
    const compactProfileText =
      this.compactForGeminiText(profileText) || profileText;

    const aiResponse: FitAiResponse | null =
      allowAi && this.aiEnabled
        ? await this.callGemini(compactJobText, compactProfileText)
        : null;
  ...
  private compactForGeminiText(raw: string, limit: number = 2800): string {
    if (!raw) return "";
    ...
    const tokens = raw
      ...
      .filter((token) => {
        ...
        if (AI_TEXT_STOPWORDS.has(lower)) return false;
        if (/^\d+$/.test(lower) && lower.length <= 2) return false;
        return true;
      });

    if (!tokens.length) return "";

    const compacted = tokens.join(" ").replace(/\s{2,}/g, " ").trim();
...
{"jobSkills":["React","Vue","HTML"],"profileSkills":["React","Node.js"],"matchedSkills":["React"],"missingSkills":["Vue","HTML"],"matchPercentage":33,"summary":"Brief fit summary"}`
      ...
      console.log("[job-fit][gemini] request", {
        ...
        jobTextPreview: jobText.slice(0, 1500),
        profileTextPreview: profileText.slice(0, 1500),
      });
